...
| Method | Path | Purpose |
|---|---|---|
| `POST` | `/api/runs/intake` | Create a run, validate chapters, and generate the adaptation plan |
| `POST` | `/api/runs/{run_id}/generate` | Generate screenplay JSON/YAML from the plan and author controls |
| `POST` | `/api/runs` | Compatibility one-shot run from pasted text or `.txt` upload |
...
The workflow is fixed by design, but split into two author-visible phases:

1. Intake planning: detect chapters, understand story facts, and produce `adaptation_plan.json/md`.
2. Script generation: apply author controls, produce validated JSON, export YAML, and generate a report.
